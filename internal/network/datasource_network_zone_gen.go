...
func (d *NetworkZoneDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *NetworkZoneDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var state NetworkZoneDataSourceModel
	...
	networkZoneName := state.Name.ValueString()
	networkZone, _, err := server.GetNetworkZone(networkZoneName)
	if err != nil {
		resp.Diagnostics.AddError(fmt.Sprintf("Failed to retrieve existing network zone %q", networkZoneName), err.Error())
		return
	}

	config, diags := common.ToConfigMapType(ctx, common.ToNullableConfig(networkZone.Config), state.Config)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	state.Name = types.StringValue(networkZone.Name)
	state.Description = types.StringValue(networkZone.Description)
	state.Config = config
