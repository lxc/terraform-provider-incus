...
func TestAccInstance_execTriggerOnStartRunsOnStart(t *testing.T) {
	instanceName := petname.Generate(2, "-")
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config:      testAccInstance_execOnStartNoRun(instanceName, "v1"),
				ExpectError: regexp.MustCompile("Exec commands can only be run on running instances."),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("incus_instance.instance1", "running", "false"),
				),
			},
			{
				// This step fails only if the exec runs when the provider starts the instance.
				Config:      testAccInstance_execOnStartFail(instanceName, "v2"),
				ExpectError: regexp.MustCompile("Failed to execute command"),
			},
		},
	})
...
func testAccInstance_execOnStartNoRun(instanceName, description string) string {
	return fmt.Sprintf(`
resource "incus_instance" "instance1" {
  name        = "%s"
  image       = "%s"
...
func testAccInstance_execOnStartFail(instanceName, description string) string {
	return fmt.Sprintf(`
resource "incus_instance" "instance1" {
  name        = "%s"
  image       = "%s"
